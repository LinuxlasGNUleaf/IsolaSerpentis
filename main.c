...
void handle_command()
{
        ...
        else if (!strcmp(attribute, "geist") && geist) // Geist-Aktionen
        {
            ...
            else
            {
                ...
                if (room_ind == 98 && monster_defeated[FISCH])
                    ...
                    fancy_print("„Das Meer schwemmt vieles an...“", standard_reading_tempo, true);
                ...
                else
                {
                    ...
                    {
                    case 0:
                        fancy_print("„Vielleicht findest du in der Nähe einen nützlichen Gegenstand?“", standard_reading_tempo, true);
                        break;
                    case 1:
                        fancy_print("„Du solltest dir gerade jetzt mehr Mühe geben!“", standard_reading_tempo, true);
                        break;
                    case 2:
                        fancy_print("„Ich habe jetzt keine Zeit für dich! Wir feiern in Schottland ein Familienfest!“", standard_reading_tempo, true);
                        break;
                    case 3:
                        fancy_print("„Selbst Geister wissen nicht immer Rat!“", standard_reading_tempo, true);
                        break;
                    case 4:
    ...
    else if (!strcmp(command, "lese"))
    {
        if (strlen(attribute))
        {
            if (!strcmp(attribute, "buch") && room_ind == 29)
            {
                fancy_print(buchtext, standard_reading_tempo, true);
            }
        }
        else
        {
    ...
    else if (!strcmp(command, "info"))
    {
        ...
        for (int i = 0; i < OBJEKTE_LEN; i++)
        {
            ...
            if (!strcmp(attribute, buffer2) && has_item(i))
            {
                fancy_print(objekte_desc[i], standard_reading_tempo, true);
                break;
            }
            if (i == OBJEKTE_LEN - 1)
...
void print_description()
{
    if (biome[last_room - 1] != MEER && biome[room_ind - 1] == MEER)
        ...
        fancy_print("Von Meerwasser triefend kommst du wieder an Land und lässt deine Kleidung in der Sonne trocknen.", standard_reading_tempo, true);
